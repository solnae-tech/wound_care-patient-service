...
class WoundStatus(str, enum.Enum):
    OPEN = 'open'
    HEALING = 'healing'
    INFECTED = 'infected'
    CLOSED = 'closed'
...
class Wounds(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(always=True, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), primary_key=True)
    user_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    wound_type: Mapped[WoundType] = mapped_column(Enum(WoundType, values_callable=lambda cls: [member.value for member in cls], name='wound_type'), nullable=False)
    status: Mapped[WoundStatus] = mapped_column(Enum(WoundStatus, values_callable=lambda cls: [member.value for member in cls], name='wound_status'), nullable=False, server_default=text("'open'::wound_status"))
